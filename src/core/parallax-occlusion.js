...
export const parallaxOcclusionUV = ( heightMap, options = {} ) => {
	...
	// SeedThree addition: the height field may be a COMPUTED node rather than a
	// single texture's red channel — e.g. terrain that picks the dominant splat
	// layer's height per pixel. `options.height` is a function ( coord ) => Node
	// returning the surface height (white = peak, 0..1); when set, `heightMap` is
	// optional and every internal height fetch routes through it. Default keeps
	// the published behaviour (red channel of `heightMap` at explicit LOD 0).
	const heightSample = height !== null ? height : ( coord ) => textureLevel( heightMap, coord, 0 ).r;
	...
	const depthAt = ( coord ) => float( 1.0 ).sub( heightSample( coordAt( coord ) ) );
	...
	const shadow = ( lightDirection, shadowOptions = {} ) => {
		...
		const rayHeight = heightSample( coordAt( offsetUV ) ).add( bias ).toVar();
		...
		Loop( { start: 0, end: steps, type: 'int' }, () => {
			...
			const blocker = heightSample( coordAt( shadowUV ) ).sub( rayHeight );
